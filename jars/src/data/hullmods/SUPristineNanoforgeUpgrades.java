...
public class SUPristineNanoforgeUpgrades extends BaseHullMod {
   protected static Object STATUSKEY = new Object();
   public static String MA_DATA_KEY = "shu_core_pristine_reloader_data_key";
   public static final String DATA_PREFIX = "pristine_nanoforge_shu_check_";
   public static final String ITEM = "pristine_nanoforge";
   ...
   public static float ammoRegenBonus = SUPlugin.CM_PNANOFORGE_WEAPON_RELOAD_BONUS;
   public static float ammoBonus = SUPlugin.CM_PNANOFORGE_AMMO_BONUS;
   public static float missileAmmoBonus = SUPlugin.CM_PNANOFORGE_MISSILE_AMMO_BONUS;
   boolean toggleGeneralIncompat;
   private static final String[] ALL_INCOMPAT_IDS = new String[]{};


   private String[] getIncompatibleIds() {
      return super.spec.getId().equals("specialsphmod_pristinenanoforge_upgrades") ? ALL_INCOMPAT_IDS : null;
   }

   public CargoStackAPI getRequiredItem() {
      return Global.getSettings().createCargoStack(CargoItemType.SPECIAL, new SpecialItemData("pristine_nanoforge", null), null);
   }
   ...
   public void addRequiredItemSection(
      ...
      if (req != null) {
         ...
         if (isForModSpec || Global.CODEX_TOOLTIP_MODE) {
            ...
            TooltipMakerAPI text = tooltip.beginImageWithText(Global.getSettings().getSpecialItemSpec("pristine_nanoforge").getIconName(), 20.0F);
            ...
            if (currentVariant.hasHullMod(this.spec.getId())) {
               if (!currentVariant.getHullSpec().getBuiltInMods().contains(this.spec.getId())) {
                  ...
                  TooltipMakerAPI text2 = tooltip.beginImageWithText(Global.getSettings().getSpecialItemSpec("pristine_nanoforge").getIconName(), 20.0F);
               ...
               TooltipMakerAPI text3 = tooltip.beginImageWithText(Global.getSettings().getSpecialItemSpec("pristine_nanoforge").getIconName(), 20.0F);
   ...
   public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
      if (SUPlugin.HASLUNALIB) {
         ...
         ammoRegenBonus = LunaSettings.getFloat("mayu_specialupgrades", "LUNA_CM_PNANOFORGE_WEAPON_RELOAD_BONUS");
         ammoBonus = LunaSettings.getFloat("mayu_specialupgrades", "LUNA_CM_PNANOFORGE_AMMO_BONUS");
         missileAmmoBonus = LunaSettings.getFloat("mayu_specialupgrades", "LUNA_CM_PNANOFORGE_MISSILE_AMMO_BONUS");
      }
      ...
      if (this.enableCustomSM) {
         ...
         stats.getEnergyAmmoBonus().modifyPercent(id, ammoBonus);
         stats.getMissileAmmoBonus().modifyPercent(id, missileAmmoBonus);
   ...
   public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
      ...
      if (ship.getVariant().getSMods().contains("specialsphmod_pristinenanoforge_upgrades")
         || ship.getVariant().getHullSpec().isBuiltInMod("specialsphmod_pristinenanoforge_upgrades")) {
         ship.getVariant().removePermaMod("specialsphmod_pristinenanoforge_upgrades");
      }

      if (ship.getVariant().hasHullMod("specialsphmod_pristinenanoforge_upgrades")) {
         ship.getVariant().addPermaMod("specialsphmod_pristinenanoforge_utilityscript");
         if (currentShipStats != null) {
            SUHullmodUpgradeInstaller.applyHullmodToModulesOfShip(shipVariant, currentShipStats, "specialsphmod_nanoforge_pristine_extension");
         }
      }
   }
   ...
   public void advanceInCombat(ShipAPI ship, float amount) {
      ...
      if (!this.disableExtraEffect) {
         ...
         SUPristineNanoforgeUpgrades.MissileAutoforger data = (SUPristineNanoforgeUpgrades.MissileAutoforger)engine.getCustomData().get(key);
         if (data == null) {
            data = new SUPristineNanoforgeUpgrades.MissileAutoforger();
      ...
      if (!isForModSpec && ship != null) {
         ...
         if (this.enableCustomSM) {
            ...
            tooltip.addPara(
               ...
               new String[]{Misc.getRoundedValue(ammoRegenBonus) + "%", Misc.getRoundedValue(ammoBonus) + "%", Misc.getRoundedValue(missileAmmoBonus) + "%"}
            );
         } else if (!this.enableCustomSM) {
            tooltip.addPara(
               "• Decreases reload time of non-missile weapons: %s\n• Increases ammo capacity of weapons: %s\n• Increases ammo capacity of missile weapons: %s",
