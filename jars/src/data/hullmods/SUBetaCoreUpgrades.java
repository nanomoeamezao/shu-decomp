...
public class SUBetaCoreUpgrades extends BaseHullMod {
   public static final String DATA_PREFIX = "beta_core_shu_check_";
   public static final String ITEM = "beta_core";
   ...
   private static final float MAX_CR_STRKCRFT = 0.1F;
   private static final float TIME_MULT = 10.0F;
   ...
   private static final String[] INCOMPAT_CORES = new String[]{"specialsphmod_alpha_core_upgrades", "specialsphmod_gamma_core_upgrades"};
   private static final Map HULLDP = new HashMap();

   private String[] getIncompatibleIds() {
      return super.spec.getId().equals("specialsphmod_beta_core_upgrades") ? INCOMPAT_CORES : null;
   }

   public SUBetaCoreUpgrades() {
      this.ID = "SUBetaCoreUpgrades";
   }

   public CargoStackAPI getRequiredItem() {
      return Global.getSettings().createCargoStack(CargoItemType.RESOURCES, "beta_core", null);
   }
   ...
   public void addRequiredItemSection(
      ...
      if (req != null) {
         ...
         if (isForModSpec || Global.CODEX_TOOLTIP_MODE) {
            ...
            TooltipMakerAPI text = tooltip.beginImageWithText(Global.getSettings().getCommoditySpec("beta_core").getIconName(), 20.0F);
            ...
            if (currentVariant.hasHullMod(this.spec.getId())) {
               if (!currentVariant.getHullSpec().getBuiltInMods().contains(this.spec.getId())) {
                  ...
                  TooltipMakerAPI text2 = tooltip.beginImageWithText(Global.getSettings().getCommoditySpec("beta_core").getIconName(), 20.0F);
               ...
               TooltipMakerAPI text3 = tooltip.beginImageWithText(Global.getSettings().getCommoditySpec("beta_core").getIconName(), 20.0F);
   ...
   public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
      ...
      if (stats.getVariant().hasHullMod("strikeCraft")) {
         stats.getMaxCombatReadiness().modifyFlat(id, 0.1F, "Armament Support System (Beta)");
      }
   ...
   public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
      ...
      if (ship.getVariant().getSMods().contains("specialsphmod_beta_core_upgrades")
         || ship.getVariant().getHullSpec().isBuiltInMod("specialsphmod_beta_core_upgrades")) {
         ship.getVariant().removePermaMod("specialsphmod_beta_core_upgrades");
      }

      if (ship.getVariant().hasHullMod("specialsphmod_beta_core_upgrades")) {
         ship.getVariant().addPermaMod("specialsphmod_betacore_utilityscript");
         if (currentShipStats != null) {
            SUHullmodUpgradeInstaller.applyHullmodToModulesOfShip(shipVariant, currentShipStats, "specialsphmod_beta_core_module_extension");
         }
      }
   }
      ...
      if (!Global.getCombatEngine().isPaused()) {
         ...
         if (ship.isAlive() && !ship.isPiece()) {
            ...
            if (ship.getHullSize() == HullSize.FRIGATE && ship.getHullSpec().getHullId().startsWith("armaa_")
               ...
               if (player) {
                  stats.getTimeMult().modifyPercent(this.ID, 10.0F);
                  stats.getTurnAcceleration().modifyMult(this.ID, 1.3F);
                  stats.getFluxDissipation().modifyMult(this.ID, 1.3F);
                  stats.getShieldUpkeepMult().modifyMult(this.ID, 0.7F);
                  stats.getShieldTurnRateMult().modifyMult(this.ID, 1.3F);
                  stats.getShieldUnfoldRateMult().modifyMult(this.ID, 1.3F);
                  stats.getPhaseCloakUpkeepCostBonus().modifyMult(this.ID, 0.7F);
                  stats.getPhaseCloakCooldownBonus().modifyMult(this.ID, 0.7F);
                  stats.getCombatEngineRepairTimeMult().modifyMult(this.ID, 0.7F);
                  stats.getCombatWeaponRepairTimeMult().modifyMult(this.ID, 0.7F);
                  Global.getCombatEngine().getTimeMult().modifyPercent(this.ID, 0.1F);
                  Global.getCombatEngine()
                     .maintainStatusForPlayerShip(
                        this.ID + "_TOOLTIP_", Global.getSettings().getSpriteName("tooltips", "ains_system"), "Time Accel", "Timeflow increased: 10%", false
                     );
               } else {
                  stats.getTimeMult().modifyPercent(this.ID, 10.0F);
                  stats.getTurnAcceleration().modifyMult(this.ID, 1.3F);
      ...
      if (!isForModSpec && ship != null) {
         ...
         if (Global.getSettings().getModManager().isModEnabled("yunrucore")) {
            if (ship.getVariant().hasHullMod("specialsphmod_beta_core_upgrades")
               ...
               withyunrubeta.addPara(
                  "A ship with %s has been detected, and having multiple Beta cores installed together will unlock their latent synchronization ability. The following additional effects will be applied.",
                  SUStringCodex.SHU_TOOLTIP_PADZERO,
         ...
         if (Global.getSettings().getModManager().isModEnabled("armaa")) {
            if (ship.getHullSize() == HullSize.FRIGATE
               && ship.getVariant().hasHullMod("specialsphmod_beta_core_upgrades")
               && ship.getVariant().getHullMods().contains("strikeCraft")
         ...
         if (!this.toggleGeneralIncompat) {
            ...
            text.addPara(
               ...
               new String[]{"Armament Support System (Alpha)", "Armament Support System (Gamma)"}
            );
            tooltip.addImageWithText(SUStringCodex.SHU_TOOLTIP_PADMAIN);
         }
   ...
   static {
      ...
      HULLDP.put(HullSize.FRIGATE, 10.0F);
      HULLDP.put(HullSize.DESTROYER, 10.0F);
      HULLDP.put(HullSize.CRUISER, 10.0F);
      HULLDP.put(HullSize.CAPITAL_SHIP, 10.0F);
   }
}
