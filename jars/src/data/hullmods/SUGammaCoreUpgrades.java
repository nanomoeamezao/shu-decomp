...
public class SUGammaCoreUpgrades extends BaseHullMod {
   public static final String DATA_PREFIX = "gamma_core_shu_check_";
   ...
   private static final float MAX_CR_STRKCRFT = 0.05F;
   private static final float TIME_MULT = 5.0F;
   ...
   private static final String[] INCOMPAT_CORES = new String[]{"specialsphmod_alpha_core_upgrades", "specialsphmod_beta_core_upgrades"};
   private static final Map HULLDP = new HashMap();

   private String[] getIncompatibleIds() {
      return super.spec.getId().equals("specialsphmod_gamma_core_upgrades") ? INCOMPAT_CORES : null;
   }

   public SUGammaCoreUpgrades() {
      this.ID = "SUGammaCoreUpgrades";
   }

   public CargoStackAPI getRequiredItem() {
      return Global.getSettings().createCargoStack(CargoItemType.RESOURCES, "gamma_core", null);
   }
   ...
   public void addRequiredItemSection(
      ...
      if (req != null) {
         ...
         if (isForModSpec || Global.CODEX_TOOLTIP_MODE) {
            ...
            TooltipMakerAPI text = tooltip.beginImageWithText(Global.getSettings().getCommoditySpec("gamma_core").getIconName(), 20.0F);
            ...
            if (currentVariant.hasHullMod(this.spec.getId())) {
               if (!currentVariant.getHullSpec().getBuiltInMods().contains(this.spec.getId())) {
                  ...
                  TooltipMakerAPI text2 = tooltip.beginImageWithText(Global.getSettings().getCommoditySpec("gamma_core").getIconName(), 20.0F);
               ...
               TooltipMakerAPI text3 = tooltip.beginImageWithText(Global.getSettings().getCommoditySpec("gamma_core").getIconName(), 20.0F);
   ...
   public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
      ...
      if (stats.getVariant().hasHullMod("strikeCraft")) {
         stats.getMaxCombatReadiness().modifyFlat(id, 0.05F, "Armament Support System (Gamma)");
      }
   ...
   public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
      ...
      if (ship.getVariant().getSMods().contains("specialsphmod_gamma_core_upgrades")
         || ship.getVariant().getHullSpec().isBuiltInMod("specialsphmod_gamma_core_upgrades")) {
         ship.getVariant().removePermaMod("specialsphmod_gamma_core_upgrades");
      }

      if (ship.getVariant().hasHullMod("specialsphmod_gamma_core_upgrades")) {
         ship.getVariant().addPermaMod("specialsphmod_gammacore_utilityscript");
         if (currentShipStats != null) {
            SUHullmodUpgradeInstaller.applyHullmodToModulesOfShip(shipVariant, currentShipStats, "specialsphmod_gamma_core_module_extension");
         }
      }
   }
      ...
      if (!Global.getCombatEngine().isPaused()) {
         ...
         if (ship.isAlive() && !ship.isPiece()) {
            ...
            if (ship.getHullSize() == HullSize.FRIGATE && ship.getHullSpec().getHullId().startsWith("armaa_")
               ...
               if (player) {
                  stats.getTimeMult().modifyPercent(this.ID, 5.0F);
                  stats.getTurnAcceleration().modifyMult(this.ID, 1.1F);
                  stats.getFluxDissipation().modifyMult(this.ID, 1.1F);
                  stats.getShieldUpkeepMult().modifyMult(this.ID, 0.9F);
                  stats.getShieldTurnRateMult().modifyMult(this.ID, 1.1F);
                  stats.getShieldUnfoldRateMult().modifyMult(this.ID, 1.1F);
                  stats.getPhaseCloakUpkeepCostBonus().modifyMult(this.ID, 0.9F);
                  stats.getPhaseCloakCooldownBonus().modifyMult(this.ID, 0.9F);
                  stats.getCombatEngineRepairTimeMult().modifyMult(this.ID, 0.9F);
                  stats.getCombatWeaponRepairTimeMult().modifyMult(this.ID, 0.9F);
                  Global.getCombatEngine().getTimeMult().modifyPercent(this.ID, 0.2F);
                  Global.getCombatEngine()
                     .maintainStatusForPlayerShip(
                        this.ID + "_TOOLTIP_", Global.getSettings().getSpriteName("tooltips", "ains_system"), "Time Accel", "Timeflow increased: 5%", false
                     );
               } else {
                  stats.getTimeMult().modifyPercent(this.ID, 5.0F);
                  stats.getTurnAcceleration().modifyMult(this.ID, 1.1F);
      ...
      if (!isForModSpec && ship != null) {
         ...
         if (!this.disableDPCostPenalty) {
            tooltip.addPara(
               ...
               new String[]{Misc.getRoundedValue(5.0F) + "%"}
            );
         }
         ...
         if (Global.getSettings().getModManager().isModEnabled("yunrucore")) {
            if (ship.getVariant().hasHullMod("specialsphmod_gamma_core_upgrades")
               ...
               withyunrugamma.addPara(
                  "A ship with %s has been detected, and having multiple Gamma cores installed together will unlock their latent synchronization ability. The following additional effects will be applied.",
                  SUStringCodex.SHU_TOOLTIP_PADZERO,
         ...
         if (Global.getSettings().getModManager().isModEnabled("armaa")) {
            if (ship.getHullSize() == HullSize.FRIGATE
               ...
               tooltip.addPara(
                  ...
                  SUStringCodex.SHU_TOOLTIP_PADMAIN,
                  SUStringCodex.SHU_TOOLTIP_GREEN,
                  new String[]{Misc.getRoundedValue(5.0F) + "%", Misc.getRoundedValue(5.0F) + "%"}
               );
            }
         ...
         if (!this.toggleGeneralIncompat) {
            ...
            text.addPara(
               ...
               new String[]{"Armament Support System (Alpha)", "Armament Support System (Beta)"}
            );
            tooltip.addImageWithText(SUStringCodex.SHU_TOOLTIP_PADMAIN);
         }
